class graph():
    def __init__(self,initial_dic={}):
        ...
        if(len(initial_dic)!=0):
            ...
            self.maximum_node=-1
        self.nodes=[0 for i in range(self.maximum_node)]

    def AddNode(self,node):
        '''note that nodes must be positive'''
        self.network[node]=set({})
        if(node>self.maximum_node):
            for i in range(self.maximum_node,node):
                self.nodes.append(0)
            self.nodes[node-1]=1
    ...
    def StartMarking(self):
        ...
        self.marks=[ 0 for i in range(self.maximum_node)]

    def Mark(self,node,color):
        self.marks[node-1]=color

    def GetMark(self,node):
        return self.marks[node-1]
    ...
    def _FindPath(self, start, dest):
        ...
        visited=[ 0 for i in range(self.maximum_node)]
